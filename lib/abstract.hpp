...
inline uint32_t hash32(data_type item, uint32_t seed) {
	return MurmurHash2((uint8_t *)&item, sizeof(data_type), seed);
}
...
public:
	...
	virtual count_type PointQuery(const data_type item) = 0;
    virtual void Query(int threshold, myvector results) = 0;
