...
void mangle(const unsigned char* key, unsigned char* ret_key,
    ...
    for (i = 0; i < nbytes; ++i) {
        if (i == 0) {
            temp = key[0];
            new_key |= (temp&0x0f);
            new_key |= ((temp&0xf0)<<12);
        } else if (i == 1) {
            temp = key[1];
            new_key |= ((temp&0x0f)<<4);
            new_key |= ((temp&0xf0)<<16);
        } else if (i == 2) {
            temp = key[2];
            new_key |= (temp&0x0f)<<8;
            new_key |= (temp&0xf0)<<20;
        } else {
            temp = key[3];
            new_key |= (temp&0x0f)<<12;
            new_key |= (temp&0xf0)<<24;
        }
    }
    memcpy(&temp, key, nbytes);
...
void unmangle(const unsigned char* key, unsigned char* ret_key,
    ...
    for (i=0; i<nbytes; ++i) {
        if(i == 0) {
            ret_key[0] = new_key & 0x0f;
            ret_key[0] |= ((new_key >> 12) & 0xf0);
        } else if(i == 1) {
            ret_key[1] = (new_key >> 4) & 0x0f;
            ret_key[1] |= (new_key >> 16) & 0xf0;
        } else if(i == 2) {
            ret_key[2] = (new_key >> 8) & 0x0f;
            ret_key[2] |= (new_key >> 20) & 0xf0;
        } else {
            ret_key[3] = (new_key >> 12) & 0x0f;
            ret_key[3] |= (new_key >> 24) & 0xf0;
        }
    }
...
void mangle(const unsigned char* key, unsigned char* ret_key,
    ...
    //invert key, lower 8 bits becomes higher 8 bits
    for (i=0; i<nbytes; ++i) {
