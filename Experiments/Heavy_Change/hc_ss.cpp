...
int main(int argc,char* argv[]) {
    ...
    int k_num = (memory * 1024 - 40) / 56;
    k_num = (k_num & 1) ? k_num : k_num - 1;
    ...
    for (std::string file; getline(tracefiles, file);) {
        ...
        SpaceSaving *sketch1 = new SpaceSaving(k_num);
        SpaceSaving *sketch2 = new SpaceSaving(k_num);
        sketch1->Reset();
        sketch2->Reset();
        ...
        // for (auto i = truth.begin(); i != truth.end(); i++)
        // {
        //     int val = sketch->PointQuery(key);
        // }
    ...
    txtfile << std::setw(20) << std::left <<  "SS"
        << std::setw(20) << std::left << memory*2
        << std::setw(20) << std::left << threshold
